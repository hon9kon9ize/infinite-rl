...
impl Qwen3TextEmbedding {
    ...
    pub fn from_hf(
        ...
        let api = ApiBuilder::new()
            .with_progress(true)
            .build()
            .map_err(|e: hf_hub::api::sync::ApiError| candle_core::Error::Msg(e.to_string()))?;
        ...
        let cfg: Config = serde_json::from_slice(
            &std::fs::read(&cfg_path)
                .map_err(|e: std::io::Error| candle_core::Error::Msg(e.to_string()))?,
        )
        .map_err(|e: serde_json::Error| candle_core::Error::Msg(e.to_string()))?;
    ...
    /// Load from HF and also write a local cache directory containing:
    /// - `config.json`
    /// - `tokenizer.json`
    /// - `model.safetensors` (or sharded files)
    #[cfg(feature = "hf-hub")]
    pub fn from_hf_cached(
        repo_id: &str,
        device: &Device,
        dtype: DType,
        _max_length: usize,
        ...
        eprintln!("--- Starting Qwen3 Initialization ---");

        // 1. Efficient Local Load Check
        if cache_dir.exists() {
            let step_start = Instant::now();

            let cfg_file = cache_dir.join("config.json");
            let tok_file = cache_dir.join("tokenizer.json");
            let mut local_weights = Vec::new();

            // Check for canonical file first
            let canonical = cache_dir.join("model.safetensors");
            if canonical.exists() && fs::metadata(&canonical).map(|m| m.len()).unwrap_or(0) > 0 {
                local_weights.push(canonical);
            } else {
                // Check for shards only if canonical doesn't exist
                if let Ok(entries) = fs::read_dir(cache_dir) {
                    for entry in entries.flatten() {
                        let p = entry.path();
                        if p.extension().and_then(|s| s.to_str()) == Some("safetensors") {
                            if fs::metadata(&p).map(|m| m.len()).unwrap_or(0) > 0 {
                                local_weights.push(p);
                            }
                        }
                    }
                }
            ...
            local_weights.sort();
            eprintln!("Step 1: File discovery took: {:?}", step_start.elapsed());

            if cfg_file.exists() && tok_file.exists() && !local_weights.is_empty() {
                local_weights.sort(); // Ensure deterministic order for VarBuilder
                let cfg_bytes =
                    fs::read(&cfg_file).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
                let cfg: Config = serde_json::from_slice(&cfg_bytes)
                    .map_err(|e| candle_core::Error::Msg(e.to_string()))?;

                eprintln!("Step 2: Config load/parse took: {:?}", step_start.elapsed());

                let tokenizer = tokenizers::Tokenizer::from_file(&tok_file)
                    .map_err(|e| candle_core::Error::Msg(e.to_string()))?;

                eprintln!("Step 3: Tokenizer load took: {:?}", step_start.elapsed());

                // Memory-map the weights directly from the existing cache
                let vb =
                    unsafe { VarBuilder::from_mmaped_safetensors(&local_weights, dtype, device)? };
                let model = Qwen3Model::new(cfg, vb)?;

                eprintln!("Step 4: VarBuilder mmap took: {:?}", step_start.elapsed());
        ...
        // 2. Download Fallback (Only runs if cache is missing/incomplete)
        fs::create_dir_all(cache_dir).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        let api = ApiBuilder::new()
            .with_progress(true)
            .build()
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        ...
        // Download and move files (using rename or copy ONLY when necessary)
        let cfg_src = repo
            .get("config.json")
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        fs::copy(&cfg_src, cache_dir.join("config.json")).ok();

        let tok_src = repo
            .get("tokenizer.json")
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        fs::copy(&tok_src, cache_dir.join("tokenizer.json")).ok();

        // Weight handling: Determine files to download
        let weight_files = if let Ok(p) = repo.get("model.safetensors") {
            vec![p]
        } else {
            // ... (sharded download logic from your original code)
            vec![]
        };

        let mut local_paths = Vec::new();
        for wf in weight_files {
            let dest = cache_dir.join(wf.file_name().unwrap());
            if !dest.exists() {
                fs::copy(&wf, &dest).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
            }
            local_paths.push(dest);
        }

        let vb = unsafe { VarBuilder::from_mmaped_safetensors(&local_paths, dtype, device)? };
        let cfg_bytes = fs::read(cfg_src).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        let cfg: Config = serde_json::from_slice(&cfg_bytes)
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
    ...
    pub fn from_local_cache(
        ...
        if !cfg_file.exists() || !tok_file.exists() {
            return Err(candle_core::Error::Msg(format!(
                "cache missing required files in {}",
                cache_dir.display()
            )));
        }

        // 1. Collect weight paths (Logic remains the same for discovering files)
        let mut local_weights: Vec<PathBuf> = Vec::new();
        let canonical = cache_dir.join("model.safetensors");
        if canonical.exists() {
            local_weights.push(canonical);
        } else {
            for entry in
                fs::read_dir(cache_dir).map_err(|e| candle_core::Error::Msg(e.to_string()))?
            {
                let p = entry
                    .map_err(|e| candle_core::Error::Msg(e.to_string()))?
                    .path();
                if let Some(fname) = p.file_name().and_then(|s| s.to_str()) {
                    if fname.starts_with("model-") && fname.ends_with(".safetensors") {
                        local_weights.push(p);
                    }
                }
            }
        ...
        if local_weights.is_empty() {
            return Err(candle_core::Error::Msg("No weight files found".into()));
        }

        // 2. Load metadata (config/tokenizer)
        let cfg_bytes = fs::read(&cfg_file).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        let cfg: Config = serde_json::from_slice(&cfg_bytes)
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
        let tokenizer = tokenizers::Tokenizer::from_file(&tok_file)
            .map_err(|e| candle_core::Error::Msg(e.to_string()))?;

        // 3. Conditional VarBuilder: Mmap for Native, Buffered for WASM
        #[cfg(target_arch = "wasm32")]
        let vb = {
            // WASI does not support mmap. We must read the safetensors into memory buffers.
            // For simplicity, we assume a single model.safetensors here.
            // If sharded, you would need to concatenate or load multiple buffers.
            let bytes =
                fs::read(&local_weights[0]).map_err(|e| candle_core::Error::Msg(e.to_string()))?;
            VarBuilder::from_buffered_safetensors(bytes, dtype, device)?
        };

        #[cfg(not(target_arch = "wasm32"))]
        let vb = unsafe {
            // Native performance: use memory-mapping
            VarBuilder::from_mmaped_safetensors(&local_weights, dtype, device)?
        };
