...
class BiPermutations(PuzzleGenerator):
    ...
    def sol(prices0, prices1, heights0, heights1):
        ...
        orders = [
            sorted(range(n), key=lambda i: (prices0[i], heights0[i])),
    ...
    def gen_random(self):
        ...
        P = sorted(
            self.random.choices(range(1 + n // 10), k=n)
        ...
        self.add(
            dict(prices0=prices0, heights0=heights0, prices1=prices1, heights1=heights1)
        )
...
class OptimalBridges(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        heights = [
            self.random.randint(
                0, H - (max([dists[max(0, i - 1)], dists[min(m - 1, i)]]) + 1) // 2
            )
            for i in range(m + 1)
        ]
        xs = []
        ...
        indices = OptimalBridges.sol(
            H, alpha, beta, xs, ys, None
        )  # compute min-cost, thresh is ignored
        cost = alpha * (H - ys[0])
        for i, j in zip(indices, indices[1:]):
            a, b, r = xs[i], xs[j], (xs[j] - xs[i]) / 2
            assert max(ys[i], ys[j]) + r <= H, "Bridge too tall"
...
class CheckersPosition(PuzzleGenerator):
    ...
    def sol(transcript):
        ...
        for t, move in enumerate(transcript):
            ...
            if any(
                (y2 - y1) * sign < 0 for (x1, y1), (x2, y2) in zip(move, move[1:])
            ):  # backward move!
                if p.promoted >= t:
            ...
            for a, b in zip(move, move[1:]):
                ...
                if abs(x2 - x1) == 2:  # jump
                    ...
                    permuted_opts[mid].opts -= {
                        0,
                        sign,
                        2 * sign,
                    }  # Can only jump over piece of opposite sign
        ...
        def helper():  # returns True if success and store everything, otherwise None
            ...
            for t, move in enumerate(transcript):
                if abs(move[0][0] - move[1][0]) == 1:  # not a jump
                    check_no_jumps = [
                        a
                        for a, p in my_opts.items()
                        if p.jumped >= t and p.opts <= {sign, 2 * sign}
                    ]
                else:
                    for a, b in zip(move, move[1:]):
                ...
                for x, y in check_no_jumps:
                    ...
                    assert o * sign > 0
                    dys = (
                        [o] if (abs(o) == 1 and p.promoted >= t) else [-1, 1]
                    )  # only check forward jumps
                    for dx in [-1, 1]:
                        for dy in dys:
                            target_o = my_opts.get((x + 2 * dx, y + 2 * dy))
                            if target_o is not None and (
                                0 in target_o.opts or target_o.jumped < t
                            ):
                                mid_o = my_opts[x + dx, y + dy]
                                if mid_o.jumped > t and mid_o.opts <= {
                                    -sign,
                                    -2 * sign,
                                }:  # ok if jumped at t
    ...
    def gen_random(self):
        ...
        transcript = full_transcript[
            a:b
        ]  # won't add duplicates so empty transcript is only added once
    ...
    def random_game_transcript(self):
        ...
        transcript = []
        board = {
            (x, y): (1 if y < 3 else -1 if y > 4 else 0)
            for x in range(8)
            for y in range(8)
            if (x + y) % 2 == 0
        }  # new board
        ...
        def make_random_move(
            sign,
        ):  # returns True if move was made else False if no legal move
            ...
            if abs(board[end]) == 1 and any(y in {0, 7} for x, y in move[1:]):
                ...
                ] *= 2  # promotion to king, king me, at the end of turn after any jumps are done!
            return True
...
class MatchingMarkers(PuzzleGenerator):
    ...
    def gen_random(self):
        pool = self.random.sample(
            "abcdefghijklmnopqrstuvwxyz", self.random.randrange(1, 26)
        )
