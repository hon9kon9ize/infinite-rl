...
class ZipStr(PuzzleGenerator):
    ...
    def sol(substrings):
        ...
        return "".join(
            [(s[i] if i < len(s) else " ") for i in range(m) for s in substrings]
        )

    def gen_random(self):
        substrings = [
            self.random.pseudo_word() for _ in range(self.random.randrange(1, 5))
        ]
...
class SubstrCount(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        candidates = [
            ...
            if len(string[i:j]) > 1 and string.count(string[i:j]) > 2
        ]
        if not candidates:
            return
    ...
    def gen_random(self):
        ...
        while depth > 0 or self.random.randrange(10):
            t += self.random.choice(
                [self.random.pseudo_word(min_len=0, max_len=3), "(", "("]
                + [")", ")", ")"] * (depth > 0)
            )
