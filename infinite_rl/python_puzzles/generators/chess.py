...
class EightQueensOrFewer(PuzzleGenerator):
    ...
    def gen_random(self):
        m, n = [
            self.random.randrange(4, self.random.choice([10, 100])) for _ in range(2)
        ]
        if min(m, n) <= 8:
...
class KnightsTour(PuzzleGenerator):
        ...
        for seed in range(100):
            ...
            while True:
                ...
                if not candidates:
                    break
                ans.append(
                    min(candidates, key=lambda z: len(possible(*z)) + r.random())
                )
                free.remove(ans[-1])
    ...
    def gen_random(self):
        m, n = [
            self.random.randrange(5, self.random.choice([10, 100])) for _ in range(2)
        ]
        if max(m / n, n / m) <= 2:
            self.add(dict(m=m, n=n))
...
class UncrossedKnightsPath(PuzzleGenerator):
    ...
    def gen_random(self):
        m, n = [
            self.random.randrange(3, self.random.choice([10, 100])) for _ in range(2)
        ]
        k = min(m, n)
    ...
    def gen(self, target_num_instances):
        for n in self.unsolved_nxn_records:
            if self.num_generated_so_far() >= target_num_instances:
                return
            self.add(
                dict(m=n, n=n, target=self.unsolved_nxn_records[n] + 1)
            )  # Note the +1 means breaking the record!
