...
class TowersOfHanoiArbitrary(PuzzleGenerator):
    ...
    def sol(source, target):
        ...
        assert (
            disks == set(final)
            and all(isinstance(i, int) for i in state)
            and len(source) == len(target) >= 3
        )
        ans = []
...
class LongestMonotonicSubstring(PuzzleGenerator):
    ...
    def sol(length, s):
        ...
        for i in range(n):
            try:
                dyn.append(
                    max((length + 1, i, e) for length, e, _ in dyn if s[e] <= s[i])
                )
            except ValueError:
    ...
    def gen_random(self):
        n = self.random.randrange(
            self.random.choice([10, 100, 1000])
        )  # a length between 1-10 or 1-100 or 1-1000
    ...
    def sol(target_angle):
        for h in range(1, 13):
            for m in range(60):
                hour_angle = 30 * h + m / 2
                minute_angle = 6 * m
                if abs(hour_angle - minute_angle) % 360 in [
                    target_angle,
                    360 - target_angle,
                ]:
                    return [h, m]
...
class Kirkman(PuzzleGenerator):
    ...
    def sol():
        ...
        days = [
            [list(range(15)) for _2 in range(2)] for _ in range(7)
        ]  # each day is pi, inv
        counts = {
            (i, j): (7 if j in range(k, k + 3) else 0)
        ...
        while True:
            pair = rand.choice(
                todos
            )  # choose i and j to make next to each other on some day
            if rand.randrange(2):
                pair = pair[::-1]
            ...
            (b, c), (v, w) = [
                [x for x in pi[b : b + 3] if x != i] for i, b in zip(pair, bases)
            ]
            if rand.randrange(2):
                (
                    b,
            ...
            if rand.random() <= 100 ** (gained - lost):
                ...
                if len(todos) == 0:
                    return [
                        [pi[k : k + 3] for k in range(0, 15, 3)] for pi, _inv in days
                    ]
...
class No3Colinear(PuzzleGenerator):
    ...
    def sol(side, num_points):
        ...
        if side <= 5:
            ...
            return next(
                list(coords)
                for coords in combinations(grid, num_points)
                if test(coords)
            )
        ...
        if side == 10:
            ...
            return next(
                list(mirror(coords))
                for coords in combinations(grid, side // 2)
                if test(coords) and test(mirror(coords))
            )
...
class PostageStamp(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        options = [
            self.random.randrange(1, 100) for _ in range(self.random.randrange(1, 10))
        ]
        target = sum(
            self.random.choices(options, k=self.random.randrange(1, max_stamps + 1))
        )
...
class Sudoku(PuzzleGenerator):
    ...
    def solve(puz):
        ...
        for i in range(9):
            ...
            groups.append(
                [9 * a + b + i + 26 * (i % 3) for a in range(3) for b in range(3)]
            )
        ...
        def solve_helper():
            ...
            if reduce():
                if all(len(s) == 1 for s in sets):
                    ...
                    smallest_set = min(
                        range(81),
                        key=lambda i: len(sets[i]) if len(sets[i]) > 1 else 10,
                    )
    ...
    def print_sets(sets):
        ...
        for i in range(9):
            for j in range(9):
                ans += " " + "".join(
                    str(k) if k in sets[9 * i + j] else "_" for k in range(1, 10)
                )
                if j == 2 or j == 5:
                    ans += "  | "
            if i == 8:
...
class NecklaceSplit(PuzzleGenerator):
    ...
    def sol(lace):
        ...
        return next(
            ...
            if lace[n : n + len(lace) // 2].count("r") == len(lace) // 4
        )
...
class AllPandigitalSquares(PuzzleGenerator):
    ...
    def sol():
        return [
            ...
            if sorted([int(s) for s in str(i * i)]) == list(range(10))
        ]
...
class VerbalArithmetic(
    PuzzleGenerator
):  # updated because the answer was given away in the docstring! OMG
    """
    ...
    def sat(li: List[int], words=["SEND", "MORE", "MONEY"]):
        ...
        """
        assert len(li) == len(words) and all(
            i > 0 and len(str(i)) == len(w) for i, w in zip(li, words)
        )
        assert len({c for w in words for c in w}) == len(
            {(d, c) for i, w in zip(li, words) for d, c in zip(str(i), w)}
        )
    ...
    def sol(words):
        ...
        def helper(
            pos, delta
        ):  # on success, returns True and pi has the correct values
            if pos == len(steps):
                return delta == 0
    ...
    _fixed = [["FORTY", "TEN", "TEN", "SIXTY"], ["GREEN", "ORANGE", "COLORS"]]
...
class SlidingPuzzle(PuzzleGenerator):
    ...
    def sol(start):
        ...
        def get_state(
            li,
        ):  # state is an integer with 4 bits for each slot and the last 4 bits indicate where the blank is
            ans = 0
        ...
        neighbors = [
            [
                i
                for i in [b - 1, b + 1, b + d, b - d]
                if i in range(N) and (b // d == i // d or b % d == i % d)
            ]
            for b in range(N)
        ]
