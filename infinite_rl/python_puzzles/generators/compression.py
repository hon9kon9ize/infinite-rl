...
class PackingHam(PuzzleGenerator):
    ...
    def sol(num, bits, dist):
        ...
        while True:
            ...
            if all(
                bin(seqs[i] ^ seqs[j]).count("1") >= dist
                for i in range(num)
                for j in range(i)
            ):
    ...
    def gen_random(self):
        ...
        def score(seqs):
            return min(
                bin(seqs[i] ^ seqs[j]).count("1") for i in range(num) for j in range(i)
            )

        # best of 5
        seqs = min(
            [[self.random.getrandbits(bits) for _ in range(num)] for _ in range(5)],
            key=score,
        )
        dist = score(seqs)
        if dist > 0:
