...
class Oscillators(PuzzleGenerator):
    ...
    def sol(period):
        ...
        completes = [
            [x + y * 1j for x in range(n) for y in range(n)] for n in range(30)
        ]
        ...
        for _attempt in range(10**5):
            ...
            for step in range(period * 10):
                key = sum((0.123 - 0.99123j) ** z for z in live) * 10**5
                ...
                live = {
                    z
                    for z in visible
                    if sum(z + d in live for d in deltas) in range(3 - (z in live), 4)
                }
    ...
    def gen(self, target_num_instances):
        for period in range(1, target_num_instances + 1):
            self.add(
                dict(period=period), test=(period in {1, 2, 3})
            )  # period 6 takes 30s to test
...
class ReverseLifeStep(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        live = {
            self.random.randrange(-5, 5) + self.random.randrange(-5, 5) * 1j
            for _ in range(n)
        }
        deltas = (1j, -1j, 1, -1, 1 + 1j, 1 - 1j, -1 + 1j, -1 - 1j)
        visible = {z + d for z in live for d in deltas}
