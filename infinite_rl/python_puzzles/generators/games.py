...
class Nim(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        if prod < 10**6:
            self.add(
                dict(initial_state=initial_state), multiplier=10 if prod > 1000 else 1
            )
...
class Nash(PuzzleGenerator):
    ...
    def sol(A, B, eps):
        ...
        def sat(strategies: List[List[float]], A, B, eps):
            ...
            return all(
                ...
                sum(B[i][j] * p[i] for i in range(m)) <= w + eps for j in range(n)
            )
        ...
        for _attempt in range(NUM_ATTEMPTS):
            ...
            for d in dims:
                ...
                for i in range(d):
                    s[i] = (
                        (1.0 - sum(s[:-1])) if i == d - 1 else (s[i] / tot)
                    )  # to ensure sum is exactly 1.0
                strategies.append(s)
    ...
    def gen_random(self):
        ...
        A, B = [
            [[self.random.random() for _i in range(m)] for _j in range(n)]
            for _k in range(2)
        ]
    ...
    def sol(A, eps):
        ...
        for count in range(1, MAX_ITER):
            ...
            if all(
                ...
                sum(A[i][j] * p[i] for i in range(m)) >= v - eps for j in range(n)
            ):
                return [p, q]
