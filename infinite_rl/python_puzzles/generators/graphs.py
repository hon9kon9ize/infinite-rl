...
class AnyEdge(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        edges = dedup_edges(
            [
                [self.random.randrange(n + 1), self.random.randrange(n + 1)]
                for _ in range(m)
            ]
        )
...
class PlantedClique(PuzzleGenerator):
    ...
    def sol(size, edges):
        ...
        while pools:
            indices[-1] += 1
            if indices[-1] >= len(pools[-1]) - size + len(
                pools
            ):  # since list is increasing order
                indices.pop()
                pools.pop()
                continue
            if len(pools) == size:
...
class ShortestPath(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        for _ in range(m):
            weights[self.random.randrange(n)][self.random.randrange(n)] = (
                self.random.randrange(1000)
            )
...
class AnyPath(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        # random graph:
        edges = dedup_edges(
            [self.random.randrange(n), self.random.randrange(n)] for _ in range(2 * n)
        )
        if self.sol(edges):
...
class Zarankiewicz(PuzzleGenerator):
    ...
    def sol(z, n, t):
        ...
        for edges in combinations(all_edges, z):
            edge_set = set(edges)
            if all(
                any((a, b) not in edge_set for a in left for b in right)
                for left in combinations(range(n), t)
                for right in combinations(range(n), t)
            ):
...
class GraphIsomorphism(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        g1 = sorted(
            {
                (self.random.randrange(n), self.random.randrange(n))
