...
class LearnParity(PuzzleGenerator):
    ...
    def sol(vecs):
        ...
        for i in range(d):  # zero out bit i
            ...
            assert (
                v[i] == 1
            )  # found a vector with v[i] = 1, subtract it off from those with a 1 in the ith coordinate
            w = v[:]
            for v in pool:
                if v[i] == 1:
                    for j in range(d + 1):
    ...
    def rand_parity_problem(rand, d=63):
        ...
        vecs = [
            sum(1 << i for i, b in enumerate(v) if b) for v in vecs
        ]  # encode into ints
        return vecs
...
class LearnParityWithNoise(PuzzleGenerator):
    ...
    def gen_random(self):
        d = self.random.randrange(
            2, self.random.choice([11, 100])
        )  # number of dimensions
        vecs = self.rand_parity_problem(self.random, d=d)
        self.add(
            dict(vecs=vecs),
            test=d < 19,
            multiplier=1000 if d > 40 else 30 if d >= 19 else 1,
        )
