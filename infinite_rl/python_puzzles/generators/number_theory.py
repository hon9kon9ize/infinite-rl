...
class GCD_multi(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        factor, *rs = [
            1 + self.random.randrange(10 ** self.random.randrange(10))
            for _ in range(k + 1)
        ]
...
class SmallExponentBigSolution(PuzzleGenerator):
    ...
    def gen(self, target_num_instances):
        ...
        solved = {
            b: {pow(b, n, n) for n in range(1, 10**5) if 1 < pow(b, n, n) < m}
            for b in range(2, 11)
        }
...
class FourSquares(PuzzleGenerator):
    ...
    def sol(n):
        ...
        sums_of_squares = {
            i + j: [a, b] for i, a in squares.items() for j, b in squares.items()
        }
...
class Factoring(PuzzleGenerator):
    ...
    *See class FermatComposite in codex.py for an easier composite test puzzle*
    ...
    DATA_PATH = os.path.join(
        os.path.dirname(__file__), "problem_constants", "rsa_challenges.json"
    )
    MAX_TEST = 10**16
    ...
    def gen(self, target_num_instances):
        with open(self.DATA_PATH) as f:
            challenges = json.load(
                f
            )  # format: ["challenge name", "n factor"] (if factor is known)
    ...
    def gen_random(self):
        ...
        n = (2 * self.random.randrange(2, m) + 1) * (
            2 * self.random.randrange(2, m) + 1
        )
        self.add(dict(n=n), test=n < self.MAX_TEST)
...
class CollatzDelay(PuzzleGenerator):
    ...
    * if n is odd, multiply n by 3 and add 1
    ...
    def gen(self, target_num_instances):
        nums = [1000, 2000, 2283, 2337, 2350, 2500, 3000, 4000] + list(
            range(target_num_instances)
        )

        for t in nums:
            if self.num_generated_so_far() < target_num_instances:
                self.add(
                    dict(t=t, upper=t // 15 + self.random.randint(30, 100)),
                    test=t <= 100,
                )


# COMMENTED OUT: Lehmer requires arbitrary-precision arithmetic for large integers. --- IGNORE ---
