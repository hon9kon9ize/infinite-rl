...
class StrCount(PuzzleGenerator):
    ...
    def sol(substring, count, length):
        c = chr(
            1 + max(ord(c) for c in (substring or "a"))
        )  # a character not in substring
...
class StrSplitter(PuzzleGenerator):
    ...
    def gen_random(self):
        ...
        parts = [
            ...
            for _ in range(1, self.random.randrange(6))
        ]
...
class ListMul(PuzzleGenerator):
    ...
    def gen_random(self):
        li = [
            self.random.randrange(-(10**5), 10**5)
...
class ListAt(PuzzleGenerator):
    ...
    def gen_random(self):
        li = [
            self.random.randrange(-(10**2), 10**2)
            for _ in range(self.random.randrange(1, 20))
        ]
