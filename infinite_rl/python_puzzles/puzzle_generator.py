...
def my_exec(cmd, globals=None, locals=None, description="source string"):
    ...
    raise InterpreterError(
        "%s at line %d of %s: %s\n%s"
        % (error_class, line_number, description, detail, cmd_str)
    )
...
def gen_dump_code(var_name: str, ty: type) -> str:
    ...
    if tys.startswith("Set["):
        return (
            "print(json.dumps({k : 1 for k in "
            + var_name
            + "})) # write sets as dictionaries\n"
        )
    ...
    if tys.startswith("Set["):
        ...
        if num_lists == 0:
            ...
            if num_lists == 1:
                ...
                if num_lists == 2:
                    assertions.append(
                        f"all(isinstance(y, {inside}) for x in {var_name} for y in x)"
                    )
                elif num_lists == 3:
                    assertions += [
                        f"all(isinstance(y, list) for x in {var_name} for y in x)",
...
class BuilderRandom(random.Random):
    ...
    def pseudo_word(self, min_len=1, max_len=20):
        w = "".join(
            self.choice(["text", "th", "ch", "qu", *"bcdfghjklmnprstvwxz"])
            + self.choice("aeiyou")
            for _ in range(1 + max_len // 2)
        )
        return w[: self.randrange(min_len, max_len + 1)]

    def heavy_tail_float(
        self, lower=-1000.0, upper=1000.0, median_dev=1.0
...
def same_types(obj1, obj2):
    ...
    if t in {list, set, dict}:
        for iterables in (
            [(obj1, obj2), (obj1.values(), obj2.values())]
            if t is dict
            else [(obj1, obj2)]
        ):
    ...
    if t is tuple:
        return len(obj1) == len(obj2) and all(
            same_types(o1, o2) for o1, o2 in zip(obj1, obj2)
        )
    return True
...
def unindent(docstr):
    ...
    for i in range(1, len(lines)):
        ...
        if de_indent and len(line) > de_indent:
            assert not line[
                :de_indent
            ].strip(), f"Weird indentation in docstring:\n{docstr}"
...
class Tags(abc.ABC):
    ...
    famous = (
        "famous"  # roughly at the level that there is a Wikipedia page about the topic
    )
...
class PuzzleGenerator:
    ...
    def get_example(cls):
        if not hasattr(cls, "_example"):
            ...
            if p_spec.defaults:
                cls._example = dict(
                    zip(p_spec.args[-len(p_spec.defaults) :], p_spec.defaults)
                )
            else:
                cls._example = {}
    ...
    def debug_problems(cls, target_num_instances=None):
        ...
        if debug_problems:
            for P in debug_problems:
                P().debug(**defaults)
            print(
                f"PuzzleGenerator.DEBUG=True problem(s) succeeded: {[p.__name__ for p in debug_problems]}"
            )
            print("Next, remove `DEBUG=True` from these classes")
        else:
            print(
                "Suggestion for debugging: set DEBUG=True on PuzzleGenerator classes to test a single class."
            )
            print(
                f"No DEBUG=True PuzzleGenerator classes found, so testing {len(all_gens):,} classes:"
            )
            for P in all_gens:
                P().test(**defaults)
    ...
    def __init__(self):
        ...
        assert all(
            t in Tags._all_tags for t in self.tags
        ), f"invalid tag(s) in {self.name}"
        ...
        assert (
            self.ans_name in sat_spec.annotations
        ), f"Missing type hint for {self.name}.sat({self.ans_name}: ???"
        self.ans_type = type_str(sat_spec.annotations[self.ans_name])
        assert (
            self.ans_type.replace("List[", "").replace("]", "")
            in "bool float int str".split()
        ...
        for sol in self.sols:  # check solution headers and extract bodies
            ...
            assert (
                self.arg_names == sol_spec.args
            ), f"mismatched problem/solution arguments for {self.name}"
            assert (
                not sol_spec.defaults
            ), f"Don't set default parameter values for {self.name}.sol -- we'll do it"
            self.sol_bodies.append(get_body(sol_src))

        assert set(self.arg_names) == set(
            self.get_example()
        ), f"Bad {self.name} example"
        ...
        assert all(
            isinstance(mro_dict[k], staticmethod) for k in ["sat"] + sol_names
    ...
    def test_input(self, name, inp, test: bool, multiplier: float, already_tested={}):
        ...
        """
        num_tested = 0
        new_sat_src = create_sat(
            self.sat_src, self.ans_name, self.ans_type, self.arg_names, inp
        )
        sol_header = create_sol_header(inp)

        instance = Instance(
            name, new_sat_src, sol_header, self.sol_bodies if test else [], multiplier
        )

        for sol_body, sol_func in zip(instance.sol_bodies, self.sols):
            if (
                new_sat_src in already_tested
                and sol_body in already_tested[new_sat_src]
            ):
                continue  # skip
            num_tested += 1
            ...
            assert type_check(
                self.ans_type, answer
            ), f"Solution returned wrong type for {self.name}"

            if self.DEBUG:
                assert (
                    self.sat(answer, **inp) is True
                ), f"Puzzle {self.name} didn't return True on `{inp}`"
            else:
                assert answer == decode(encode(answer))
                try:
                    env2 = dict(answer=answer, List=List)  # in case we screwed up env
                    my_exec(
                        instance.src + "\n" + "assert sat(answer) is True",
                        env2,
            ...
            if dur > DEFAULT_TIMEOUT * multiplier:
                utils.warn(
                    f"Took {dur}s to test {instance.name} (multiplier={multiplier})"
                )
    ...
    def build(
        ...
        while len(self._inputs) < target_num_instances:
            ...
            for _ in range(max_random_attempts):
                ...
                if n != len(self._inputs):  # added a problem
                    assert (
                        len(self._inputs) == n + 1
                    ), f"{self.name}.gen_random() generated more than one instance"
                    break
        ...
        for inp, test, multiplier in self._inputs:
            instance, n = self.test_input(
                f"{self.name}:{len(self.instances)}",
                inp,
                test,
                multiplier,
                already_tested,
        ...
        if num_tested:
            utils.info(
                f"Actually tested {num_tested:,}/{len(self.instances):,} "
    ...
    def check_for_trivial_solutions(
        self, force, already_tested
    ):  # check for trivial solutions
        example = self.get_example()
        src = create_sat(
            self.sat_src, self.ans_name, self.ans_type, self.arg_names, example
        )
        ...
        for t in tests:
            try:
                ...
                continue
            utils.warn(
                f"`{self.name}` in file `{self.__module__.split('.')[-1]}` "
        ...
        if dur > 1.0:  # warn if above one second
            utils.warn(
                f"Took {dur:.1f}s to test for trivial solutions to `{self.name}`"
            )
    ...
    def add(self, inp: dict, test=True, multiplier=1.0):
        ...
        assert set(inp) == set(
            self.arg_names
        ), f"Instance #{self.num_generated_so_far()} keys mismatch in {self.name}"
        example = self.get_example()
        for k in inp:
            v1, v2 = example[k], inp[k]
            if not same_types(v1, v2):
                utils.warn(
                    f"Instance #{self.num_generated_so_far()} variable `{k}` type mismatch in {self.name}"
                )
...
def get_src_spec(f: Callable):
    try:
        ...
        utils.error(
            "Cannot use inspect, happens in some interpreters... Try running in ipython."
        )
        raise
...
def create_sol_header(defaults, function_name="sol"):
    ...
    ans += ", ".join(
        f"{var}={utils.stringify(default)}" for var, default in defaults.items()
    )
    ans += "):"
    return ans
...
def create_sat(src, ans_name, ans_type, args, defaults, function_name="sat"):
    assert set(defaults) == set(
        args
    ), f"Add error: defaults don't match args {args} in {src}"
    ans = f"def {function_name}({ans_name}: {ans_type}"
    if args:
        ans += ", " + ", ".join(
            f"{v_name}={utils.stringify(defaults[v_name])}" for v_name in args
        )
    ans += "):\n"
    ans += get_body(src)
    return ans
