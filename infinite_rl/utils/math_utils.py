...
def _normalize(expr: str) -> str:
    ...
    expr = expr.rstrip(
        "\\"
    )  # remove trailing \\ since it indicates a new line break and will cause latex2text to stuck # pylint: disable=line-too-long
    expr = expr.replace("\\%", "%")
    expr = expr.replace("\\$", "$")
    expr = expr.replace("$", "")
    expr = expr.replace("%", "")
...
def extract_boxed_answer(solution: str):
    ...
    logging.debug(f"{solution=} in extracted_boxed_answer")
    return solution
