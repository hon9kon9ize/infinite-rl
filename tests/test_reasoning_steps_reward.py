...
class TestReasoningStepsRewardFunction(unittest.TestCase):
    ...
    def test_missing_think_tag(self):
        ...
        self.assertEqual(out.format_score, 0.0)
        self.assertEqual(out.correctness_score, 0.0)
        self.assertEqual(out.aux_score, 0.0)

    def test_single_indicator(self):
        model_output = "<think>First, we compute the sum.</think>"
        out = self.rf.compute_reward(model_output, None)
        # Aux-only: format and correctness zero, signal in aux_score
        self.assertEqual(out.format_score, 0.0)
        self.assertEqual(out.correctness_score, 0.0)
        self.assertAlmostEqual(out.aux_score, 0.1, places=5)
    ...
    def test_multiple_unique_indicators(self):
        ...
        self.assertEqual(out.format_score, 0.0)
        self.assertEqual(out.correctness_score, 0.0)
        self.assertAlmostEqual(out.aux_score, 0.2, places=5)
